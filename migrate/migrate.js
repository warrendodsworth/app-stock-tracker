...
async function savePages() {
  const localPages = await db.collection('apps/linkmate/pages').get();
  let pages = [];
...
async function addPages() {
  const localPages = await db.collection('apps/linkmate/pages');
  const promises = [];
...
async function queryStaleMates() {
  ...
  const usersRef = await db.collection('users').where('mateActive', '==', true).where('lastActiveDate', '<', twoWeeksAgo.toJSDate()).get();
...
async function duplicateDoc() {
  const colPath = `apps/linkmate/pages`;
